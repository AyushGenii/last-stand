...
function spawnZombie() {
  ...
  const speed =
    side === "left"
      ? zombieProperties.speedRange[0] +
        Math.random() *
          (zombieProperties.speedRange[1] - zombieProperties.speedRange[0])
      : -(
          zombieProperties.speedRange[0] +
          Math.random() *
            (zombieProperties.speedRange[1] - zombieProperties.speedRange[0])
  ...
  bullets.forEach((bullet, bulletIndex) => {
    zombies.forEach((zombie, zombieIndex) => {
      if (
        ...
        bullet.x + bullet.radius * 2 > zombie.x &&
        bullet.y < zombie.y + zombie.height &&
        bullet.y + bullet.radius * 2 > zombie.y
      ) {
        hitSound.play();
